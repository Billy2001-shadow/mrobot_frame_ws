...
bool Mapping::InitWithConfig() {
    std::string config_file_path = WORK_SPACE_PATH + "/config/mapping/mapping.yaml";
...
bool Mapping::InitParam(const YAML::Node& config_node) {
    ...
    pMap = new unsigned char[mapParams.width * mapParams.height]; //pMap为指向数组首个元素的指针(后面主要维护pMap)
...
nav_msgs::OccupancyGrid Mapping::GetCurrentMap() {
    ...
    Gridmap.info.resolution = mapParams.resolution; //这个通过yaml文件给mapParams.resolution置位
    ...
    Gridmap.info.origin.position.x = mapParams.origin_x;//这个通过yaml文件给mapParams.resolution置位
    Gridmap.info.origin.position.y = mapParams.origin_y;//这些参数应该在mapping类下搞定的
