...
ViewerFlow::ViewerFlow(ros::NodeHandle &nh, std::string cloud_topic) {
  ...
  optimized_odom_pub_ptr_ = std::make_shared<OdometryPublisher>(
      nh, "/optimized_odom", "odom", "/lidar", 100);
  current_scan_pub_ptr_ =
      std::make_shared<CloudPublisher>(nh, "/current_scan", "odom", 100);
  global_map_pub_ptr_ =
      std::make_shared<CloudPublisher>(nh, "/global_map", "odom", 100);
  local_map_pub_ptr_ =
      std::make_shared<CloudPublisher>(nh, "/local_map", "odom", 100);
  // viewer
...
bool ViewerFlow::PublishLocalData() {
  optimized_odom_pub_ptr_->Publish(
      viewer_ptr_->GetCurrentPose()); //优化后的位姿
