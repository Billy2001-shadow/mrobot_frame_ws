...
BackEndFlow::BackEndFlow(ros::NodeHandle& nh) {
    ...
    //发布
    transformed_odom_pub_ptr_ = std::make_shared<OdometryPublisher>(nh, "/transformed_odom", "odom", "/base_footprint", 100);//优化后的位姿？
    key_frame_pub_ptr_ = std::make_shared<KeyFramePublisher>(nh, "/key_frame", "odom", 100);//关键帧(最新的)
    key_frames_pub_ptr_ = std::make_shared<KeyFramesPublisher>(nh, "/optimized_key_frames", "odom", 100);//历史关键帧
